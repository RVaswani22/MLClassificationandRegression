...
def regressionObjVal(w, X, y, lambd):
    ...
    error = p1+p2                                           
    #error_grad = np.dot(np.dot(np.transpose(w),np.transpose(X)),X) - np.dot(np.transpose(y),X) + np.transpose(lambd*w)
    #print(w.shape)
    #print(X.shape)
    #print(y.shape)
    ...
    #lw=lambd*(np.transpose(w))
    #print(yx.shape)
    #print(lw.shape)
    
    #error_grad_inter=xxtw - xty + lw
    
    error_grad_inter = wtxtx - ytx + np.transpose(lw)
    
    #print (error_grad_inter.shape)
